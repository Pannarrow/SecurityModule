 ...
 * @Date 2019-08-09
 *
 * 签名sha1查看命令 keytool -list -v -keystore %s(keystore路径)
 */
class SignCheck {
    ...
    SignCheck(Context context) {
        this.context = context;
        this.cer = getCertificateSHA1Fingerprint();
    }

    SignCheck(Context context, String realCer) {
        this.context = context;
     ...
     * @param realCer
     */
    void setRealCer(String realCer) {
        this.realCer = realCer;
    }
     ...
     */
    boolean check() {
        SecurityLog.w("SignCheck",String.format("-------\n   【realCer】:%s\n" +
                "【checkedCer】:%s\n---------",realCer,cer));
        if (this.realCer != null) {
            cer = cer.trim();
            realCer = realCer.trim();
            return cer.equals(this.realCer);
        }
        return false;
    }
}
